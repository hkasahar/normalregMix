...
#' \item{alpha}{maxm by maxm matrix, whose i-th column is a vector of alphas
#' estimated given the null hypothesis m_0 = i}
#' \item{mu}{maxm by maxm matrix, whose i-th column is a vector of mus
#' estimated given the null hypothesis m_0 = i}
#' \item{sigma}{maxm by maxm matrix, whose i-th column is a vector of sigmas
#' estimated given the null hypothesis m_0 = i}
#' \item{beta}{A list of length maxm, whose i-th element is
#' a q times i matrix of betas estimated given the null hypothesis m_0 = i}
#' \item{gam}{maxm by maxm matrix, whose i-th column is a vector of gammas
#' estimated given the null hypothesis m_0 = i}
#' \item{emstat}{A maxm vector of values of modified EM statistics of
#' the model at m_0 = 1, 2, ..., maxm}
#' \item{pvals}{A maxm by 3 matrix whose i-th row indicates a vector of
#' p-values at k = 1, 2, 3}
#' \item{aic}{A maxm vector of Akaike Information Criterion of
#' the fitted model at m_0 = 1, 2, ..., maxm}
#' \item{bic}{A maxm vector of Bayesian Information Criterion of
#' the fitted model at m_0 = 1, 2, ..., maxm}
#' \item{loglik}{A maxm vector of log-likelihood values of
#' the model at m_0 = 1, 2, ..., maxm}
#' \item{penloglik}{A maxm vector of penalized log-likelihood values of
#' the model at m_0 = 1, 2, ..., maxm}
#' @examples
#' data(faithful)
#' attach(faithful)
...
regmixMEMtest <- function (y, x, m = 2, z = NULL, tauset = c(0.1,0.3,0.5),
  ...
  a <- list(emstat = emstat, pvals = result$pvals, crit = result$crit,
            parlist = regmix.pmle.result$parlist,
