                          ...
                          verb)
{
  ...
  out.short <- suppressWarnings(regmixpmle_z2(b0, y, x, ztilde, mu0h, sigma0h, m1, p, an, maxit.short,
  ...
  out <- suppressWarnings(regmixpmle_z2(b1, y, x, ztilde, mu0h, sigma0h, m1, p, an, maxit, ninits, epsilon, tau, h, k))
  ...
  for (k in 2:3) {
    ...
    maxit <- 2
    # Two EM steps
    out <- suppressWarnings(regmixpmle_z2(b, y, x, ztilde, mu0h, sigma0h, m1, p, an, maxit, ninits, epsilon, tau, h, k))
...
regmixPMLE <- function (y, x, m = 2, z = NULL, vcov.method = c("Hessian", "OPG", "none"),
  ...
  if (m == 1) {
    ...
    out.short <- suppressWarnings(regmixpmle_z2(b0, y, x, ztilde, mu0, sigma0, m, p, an, maxit.short,
                                  ninits.short, epsilon.short))
    # long EM
    indices <- order(out.short$penloglikset, decreasing = TRUE)[1:ninits]
    b1 <- b0[ ,indices] # b0 has been updated
    out <- suppressWarnings(regmixpmle_z2(b1, y, x, ztilde, mu0, sigma0, m, p, an, maxit, ninits, epsilon))
...
regmixVcov <- function(y, x, coefficients, z = NULL, vcov.method = c("Hessian", "OPG")) {
  ...
  if (m == 1) {
    ...
    if (is.null(z)) 
      out.p <- suppressWarnings(regmixpmle_z2(b, y, x, matrix(0), mu0, sigma0, m, p, an, maxit, ninits, epsilon, tau, h, k))
    else 
	{
      out.p <- suppressWarnings(regmixpmle_z2(b, y, x, z, mu0, sigma0, m, p, an, maxit, ninits, epsilon, tau, h, k))
      # Adjust y
      y <- y - z %*% gam
    }
