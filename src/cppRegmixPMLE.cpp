...
//' @title cppRegmixPMLE
//' @name  cppRegmixPMLE
//' @param bs (3+q)*m by ninits matrix of initial values of (alpha,mu,beta,sigma).
...
List cppRegmixPMLE(NumericMatrix bs,
  ...
  for (int jn=0; jn<ninits; jn++) {
    ...
    sigma  = b_jn.subvec((q1+1)*m,(q1+2)*m);
    for (int j=0; j < m; ++j){
      for (int i=0; i<q1; ++i){
    ...
    for (int iter = 0; iter < maxit; iter++) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        r = (1.0/sigma) % ( y(i) - trans(x1.row(i)*mubeta) );
      ...
      for (int j = 0; j < m; j++) {
        ...
        mubeta.col(j) = solve( design_matrix , trans(xtilde) * y );
        //mubeta.col(j) = solve( trans(xtilde) * x1 , trans(xtilde) * y );
        ssr_j = sum( trans(w.row(j)) % pow(  y - x1*mubeta.col(j) , 2 ) );
    ...
    b_jn.subvec((q1+1)*m,(q1+2)*m) = sigma;
